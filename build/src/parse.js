 ...
 */
function saveResults(site, vacancies) {
 ...
 * Вывести статистику
 */
function printStatistics(vacancies) {
    console.log('\n' + '='.repeat(60));
...
async function main() {
    ...
    if (!siteArg || siteArg === '--help' || siteArg === '-h') {
        ...
        getAvailableParsers().forEach((site) => {
            const config = getParserConfig(site);
    ...
    try {
        ...
        const siteConfig = getParserConfig(site);
        ...
        result.vacancies.slice(0, 5).forEach((vacancy, index) => {
            ...
            if (vacancy.experience)
                console.log(`   💼 ${vacancy.experience}`);
