...
export class RabotaMdParser {
    ...
    async parse(config) {
        try {
            ...
            const allVacancies = await this.parseAllPages(professionLink, config.maxPages || 10, config.delay || 1000);
            log(`\n${'='.repeat(60)}`);
     ...
     */
    async parseAllPages(professionUrl, maxPages, delay) {
        const allVacancies = [];
        let currentPage = 1;
     ...
     */
    findProfessionLink(html, searchQuery) {
        const dom = new JSDOM(html);
        const document = dom.window.document;
        // Упрощённый поиск контейнера профессий через прямой селектор
        const targetContainer = document.querySelector('#main .content-container.px-3.lg\\:px-0.pt-5.sm\\:pt-6');
        if (!targetContainer) {
            return null;
        }
     ...
     */
    extractInfoByIcon(infoBlock, iconName) {
        if (!infoBlock)
            return undefined;
...
//# sourceMappingURL=rabotaMd.js.map